...
Push-Location $root
go mod tidy
if ($LASTEXITCODE -ne 0) { Write-Host "[ERROR] go mod tidy failed"; Pop-Location; exit 1 }

Write-Host ""
Write-Host "[3/3] Building..."
$ldflags = "-s -w -X main.supabaseURL=$supabaseUrl -X main.supabaseAnonKey=$supabaseKey"
$output = Join-Path $root "gg-tracker.exe"
...
$urlFound = Select-String -Path $output -Pattern ([regex]::Escape("vydawdpzfpmwqmvymwsi")) -Quiet
$keyFound = Select-String -Path $output -Pattern ([regex]::Escape($supabaseKey.Substring(0, 30))) -Quiet
