...
GOOS=darwin GOARCH=arm64 go build -C "$ROOT" -ldflags "$LDFLAGS" -o "$ROOT/gg-tracker" . || { echo "[ERROR] Build failed"; exit 1; }
...
if strings "$ROOT/gg-tracker" | grep -q "vydawdpzfpmwqmvymwsi"; then
    echo "[OK] SUPABASE_URL is embedded"
else
    echo "[FAIL] SUPABASE_URL NOT found in binary!"
fi
if strings "$ROOT/gg-tracker" | grep -q "${SUPABASE_KEY:0:30}"; then
