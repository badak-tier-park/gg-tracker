...
GOOS=darwin GOARCH=amd64 go build -ldflags "$LDFLAGS" -o gg-tracker-intel . || { echo "[ERROR] Build failed"; exit 1; }
...
if strings gg-tracker-intel | grep -q "vydawdpzfpmwqmvymwsi"; then
    echo "[OK] SUPABASE_URL is embedded"
else
    echo "[FAIL] SUPABASE_URL NOT found in binary!"
fi
if strings gg-tracker-intel | grep -q "${SUPABASE_KEY:0:30}"; then
